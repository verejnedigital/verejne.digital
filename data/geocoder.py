...
class Geocoder:
    ...
    def __init__(self, db_address_cache, db_address_id, test_mode):
        ...
        with open("/tmp/geocode_key.txt", 'r') as f:
            self.geocode_key = yaml.load(f, Loader=yaml.FullLoader)['key']
    ...
    def AddToCache(self, address, formatted_address, lat, lng, update_formatted_address):
        ...
        if len(self.cache) < 10:
            print("Cached:", address, formatted_address, lat, lng)
            print("Cached:", list(self.cache.keys()))

    def NormalizeAddress(self, address):
        """ Simple syntactic normalization of an address."""
        return address.lower().strip()

    # !!!ACHTUNG!!!: Whenever the keys generated by this method are changed, one
    # needs to manually clear the address_cache.AddressHints otherwise the code
    # would still default to the old cached keys.
    ...
    def GeocodingAPILookup(self, address):
        ...
        if self.api_lookups > 100000:
            ...
            # and fail early.
            return None
        if self.test_mode and self.api_lookups > 10:
            return None
        params = {
            'address': address,
            'region': 'sk',
    ...
    def GetAddressId(self, address):
        ...
        def LookupKeysInCache(keys):
            ...
            for key in keys:
                ...
                with self.db_address_id.dict_cursor() as cur_id:
                    ...
                    if row_id is None:
                        return self.db_address_id.add_values("Address", [lat, lng, formatted_address]), key
        ...
        if len(address.strip()) <= 3:
            return None
        # See if there is a key_hint int the table. If so, add it as the first key
